<?php
	...
	if ($ubah = mysqli_prepare($conn, $sql)) {
		...
		if (mysqli_stmt_execute($ubah)) {
			echo '<script>alert("Password berhasil di ubah !");window.location.replace("../index.php");</script>';
		} else {
			echo '<script>alert("Telah terjadi kesalahan !");window.location.replace("../index.php");</script>';
		}
	}
